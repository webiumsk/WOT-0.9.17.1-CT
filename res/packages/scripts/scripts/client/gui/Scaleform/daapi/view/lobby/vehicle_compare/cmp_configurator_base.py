...
class VehicleCompareConfiguratorBaseView(VehicleCompareConfiguratorBaseViewMeta):
    ...
    def __tryToInit(self):
        if self.isCreated() and self._container is not None and not self.__isInited:
            self._init()
        return
# okay decompyling c:\Users\PC\wotsources\files\originals\res\packages\scripts\scripts\client\gui\Scaleform\daapi\view\lobby\vehicle_compare\cmp_configurator_base.pyc 
